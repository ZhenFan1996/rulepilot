...
5. `recommend_games` 是一次性完整终态：`playerReply` 承载完整自然回答，选择理由、差异和有依据的取舍由模型连贯表达；`selections` 只绑定候选身份与支持证据，不再重复生成逐卡叙述。搜索 contract 保留玩家要求的数量；终态 `newRecommendationCount` 声明实际发布卡片的数量，不能突破搜索 contract，可为 0 而继续绑定解释所需的证据。调用后不会再进入 Agent 修正轮次。
6. 应用不写推荐理由或取舍，也不根据已有候选伪造成功回复；局部叙述未通过证据边界时，只能显示透明的降级状态并保留已核验卡片。

搜索工具用完整的 current-turn contract 表达类型、机制、标题及排除项、人数、年龄、年份、时长和复杂度。`requestedGameCount` 表示玩家明确指定的本轮新推荐数量，未指定时为 null，由 Agent 根据需求、候选质量和差异选择数量，不设固定默认款数；0 表示只查询信息而不生成推荐卡片。候选窗口独立于推荐数量，目录分页在已有足够合格候选后停止。发布门校验全部身份及证据，保留有效卡片；证据绑定可以引用多个游戏作比较，正文与卡片共用新增推荐数量限制。目录规格里的非正数缺失值在推荐目录边界统一转为未知，不作为年龄、人数或时长证据。失败会保留玩家原始要求和已核验事实，供原对话重试；失败草稿和技术错误不写入模型对话。通用主题和体验检索使用 `descriptionQueryEnglish`，对应 PostgreSQL 英文全文索引；非拉丁字母的描述检索词由工具校验拒绝并反馈给同一个 Agent。点名身份使用支持多语言别名的 `requiredTitle`，不能同时提交描述检索。描述匹配使用匹配词频排序，避免单个重复概念的覆盖密度压过多个概念的相关性；它用于软排序，不替代结构化条件或证据核验。复杂度字段区分玩家给定数值边界的 HARD 和定性偏好估计的 SOFT，只有前者排除候选。人数、年龄、年份、时长、类型及机制约束在检索和发布时共用同一份 contract，历史 profile 不会隐式变成本轮硬约束。

推荐的 DeepSeek 适配器在原生工具交互中使用 `low` 推理强度，并使用供应商支持的 `auto` 工具选择；应用仍要求有证据的回答通过终态工具发布。供应商返回的私有推理仅在当前请求的模型消息中完整续接，不进入公开流、字符串诊断、JSON 序列化或持久会话。拒绝的候选与正常工具结果都保留同一份原始 assistant 消息，避免丢失原生续接状态；此过程没有增加模型阶段。

资料查询不锁死后续推荐：一次 turn 可以先核验参考游戏，再提交完整推荐搜索；正式推荐搜索的条件提交后不可放宽。体验研究仍按该轮相关候选批量读取。已有候选的主观体验追问在研究可用时直接选择研究或基于已有 observation 诚实回答，不再先运行一个只会重排相同字段的比较阶段；离线时仍可把已核验字段整理成一次结构化比较。内部 checkpoint 可以保留未发布候选供恢复审计，但 Agent 下一轮只能看到已经展示或明确聚焦的候选，不能把内部候选当成玩家已经收到的推荐。

`recommend_games` 的工具 schema 对候选 ID 和本轮全部允许证据各保留一个扁平 enum；每条证据是否属于对应候选仍由应用边界校验，不为每个候选复制一套 `oneOf` schema。发布边界验证 BGG 身份、玩家明确的硬条件与排除项、候选和证据归属。additive unknown JSON 字段被忽略。至少一个候选通过身份和硬条件时，安全子集立即发布：无效候选形成 shortfall；任何选择身份或证据绑定失效时，完整正文的支持关系也不再成立，因此省略该正文，保留独立核实的卡片并显示一次单独的降级状态，不触发新的模型调用。有效的完整正文按原文发布，不拼接应用撰写的推荐理由。每个选择的 `introduction` 是模型依据该游戏证据撰写的自然介绍，通过现有 `replyParts` 随卡片流式展示并持久化；总体 `playerReply` 保留自然判断和比较，不重复逐卡介绍。卡面与介绍在桌面并排、移动端上下排列，旧介绍同样直接展示。介绍的身份和证据绑定失效时仅省略对应介绍，其他已核验卡片介绍保留。最终是否推荐由 Agent 根据完整请求与证据判断，可声明 0 张新推荐卡片，仍须绑定正文用到的游戏证据；空选择或全部选择校验失败返回 typed publication failure。完整原参数只存在于这一次 assistant tool call，不再复制成 correction payload。

终态工具向 Agent 提供当前候选窗口内全部通过硬条件的候选身份和对应简介，供它自主取舍；最终卡片数量只约束选择结果，不提前截断可选身份或详细证据。机制保留原始 BGG 标签，不把存在合作或团队机制改写为唯一玩法模式；正向模式筛选也引用原始机制证据。身份与证据归属校验不等于逐句自然语言蕴含校验，推荐叙述的准确性仍需真实回答验收。
...
推荐各轮调用使用当前账户选择的同一模型，生成模式由现有模型配置统一决定；推荐适配器不另选终态模型、不启动竞争副本、不覆盖为强制推理模式。推荐模型的输出使用服务商默认资源限制，全部外部调用共用统一 wall-clock deadline；不额外设置会把推理和工具参数一起截断的模块 token 上限。
