...
候选的 BGG 出版方简介以 `B{bggId}:publisherDescription` 作为候选级证据进入终态判断；来源由已校验的选择证据直接投影，不依赖逐卡叙述。卡片展示已核实资料与操作，资料链接、候选核对数量和工具轨迹集中在可展开区域。历史持久会话仍可在该区域读取原有逐卡说明；新的推荐不再产生这套叙述字段。历史字段的移除条件是经过批准的前向数据迁移完成且持久记录中不再存在非空逐卡说明。后台翻译预热在同一互斥租约下优先覆盖 BGG 热榜，再继续排名库游标；排名库扫描完成后仍会刷新热榜，已存译文复用缓存，供应商暂不可用时保留排名游标。详情请求只读已有译文，缺失中文译文时明确展示原文状态。BGG 元数据翻译的当前持久身份是英文源名称、完整规范化简介、分类和机制组成的稳定 V5 摘要；V4 无序字段摘要只用于精确兼容读取，历史行保留审计但新写入和批量预热不再产生 V4 身份。
