...
搜索工具用完整的 current-turn contract 表达类型、机制、标题及排除项、人数、年龄、年份、时长和复杂度。`maximumRecommendations` 表示本轮允许的新推荐数量，明确数量时保留，未指定时使用产品默认值（默认一款）；0 表示只查询信息而不生成推荐卡片。候选窗口独立于推荐数量，目录分页在已有足够合格候选后停止。发布门校验全部身份及证据，保留有效卡片；证据绑定可以引用多个游戏作比较，正文与卡片共用新增推荐数量限制。目录规格里的非正数缺失值在推荐目录边界统一转为未知，不作为年龄、人数或时长证据。失败会保留玩家原始要求和已核验事实，供原对话重试；失败草稿和技术错误不写入模型对话。通用主题和体验检索使用 `descriptionQueryEnglish`，对应 PostgreSQL 英文全文索引；非拉丁字母的描述检索词由工具校验拒绝并反馈给同一个 Agent。点名身份使用支持多语言别名的 `requiredTitle`，不能同时提交描述检索。描述匹配用于软排序，不替代结构化条件或证据核验。人数、年龄、年份、时长、类型及机制约束在检索和发布时共用同一份 contract，历史 profile 不会隐式变成本轮硬约束。
...
候选的 BGG 出版方简介以 `B{bggId}:publisherDescription` 作为候选级证据进入终态判断；来源由已校验的选择证据直接投影，不依赖逐卡叙述。卡片展示已核实资料与操作，资料链接、候选核对数量和工具轨迹集中在可展开区域。历史持久会话仍可在该区域读取原有逐卡说明；新的推荐不再产生这套叙述字段。历史字段的移除条件是经过批准的前向数据迁移完成且持久记录中不再存在非空逐卡说明。后台翻译在同一互斥租约下优先处理 BGG 热榜，再扫描所有仍可用的缓存游戏源，覆盖排名范围之外的游戏；源摘要和持久译文决定是否完成，不再以排名翻译游标判定覆盖。排名快照不存在或扫描完成都不会关闭译文刷新。每款失败独立保留，其他游戏继续；共享供应商配额、并发容量和租约期限控制新工作。旧 `translation_next_offset` 列仅保留已部署回滚版本所需的存量状态，不再读取或推进；使用旧排名翻译协议的版本退出回滚集合后移除该列。详情请求只读已有译文，缺失中文译文时明确展示原文状态。翻译保留完整简介，结构校验失败在同一次供应商期限内向原模型反馈完整候选与错误，重复无进展时停止；有效简介不因可选分类修正失败而丢失。持久化失败不会报告完成，已缓存内容可供读取并在后续刷新时补存。BGG 元数据翻译的当前持久身份是英文源名称、完整规范化简介、分类和机制组成的稳定 V5 摘要；V4 无序字段摘要只用于精确兼容读取，历史行保留审计但新写入和批量预热不再产生 V4 身份。
...
PDF 先保留内容流中的文字块，再沿页面空白递归划分区域，按栏和行确定阅读顺序；文字和位置使用同一顺序，不按全页纵坐标混排双栏正文与侧栏。无可分割空白的重叠文字保留原始顺序，页面图像继续作为一等证据。
...
Agent 在同一会话中保留行动与反馈，每页原文只加入一次。每轮状态补充新读页面，并列出可用页、全部已读页 ID、已发布章节、未读可用页、已读但未用于章节的页和最新 observation。系统没有固定章节模板、slot ledger、canonical shard、global ordering、完整页覆盖门禁、固定 outline turn 或固定修正次数。
...
正文模型只接收可引用的规则证据和章节上下文，不重复携带整页图片识别名称与说明；扫描页的证据转录仍按原有证据身份提供。图片选择器只返回候选 ID 和对应步骤，不另写图片名称或规则说明；图题复用有引用的步骤标题，规则解释由章节正文负责。既有课程或其他功能保存的图片说明仍可读取。
...
配图的语义和几何分属不同责任：应用为每个有引用的教学步骤提供有界视觉对象候选及候选所在页的原始文字，视觉 Agent 结合原页上下文判断对应关系，只返回已选的
opaque candidate ID，或明确返回 `NO_VISUAL`。需要更聚焦的局部图时，可对允许细分的 ID 返回 `REFINE_CANDIDATE`；模型不能生成坐标或扩大区域。最终几何必须与应用提供的候选
完全一致。PDF 正文块、table 布局和固定页面象限不是插图候选；候选无结果时省略当前配图，不用粗糙大图伪装成精准插图。

`visualaid` 是独立的可选业务模块。文档页全部持久化且文档已标记 READY、已通知就绪后，ingestion 模块发布
`RenderedDocumentAvailable`；同步的可选监听不再延迟玩家使用文档。visualaid 可以把不可变 PDF 交给 Docling 一次性检测 picture/table 布局，将 provider
坐标转换成统一的左上角 0–1000 几何并持久化。teaching 只把 Docling `PICTURE` 读作插图候选，不知道 Docling HTTP、
API key 或 visualaid 表结构。按需细分由 visualaid 将父候选内的原始像素交给同一 API，再将子图映射回原页；子图必须在父区域内且严格更小。原候选始终保留，每个身份在本章只尝试细分一次，提交、轮询与结果下载共用剩余工作时限，工具结果和耗时进入任务活动记录。某一页已有 Docling 图片时，不再混入像素候选。只有该页没有 Docling 图片时，本地 OpenCV
