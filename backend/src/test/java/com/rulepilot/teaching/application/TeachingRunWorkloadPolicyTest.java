...
class TeachingRunWorkloadPolicyTest {
    ...
    void countsEverySectionFallbackReadAndItsSharedTwoCallRecovery() {
        ...
        // Three shared section calls, page interpretation/recovery, and one optional synchronous visual pass per
        // chapter. Evaluation-only critics are not part of the production admission budget.
        assertThat(demand.requiredModelCalls()).isEqualTo(247);
    }
    ...
    void doesNotInflateACompactTextLessonToAnUnrelatedConfiguredFloor() {
        ...
        assertThat(demand.requiredModelCalls()).isEqualTo(33);
    }
    ...
    void derivesAFiniteBudgetForALargerUnlikePlanWithoutAFixedPlanCeiling() {
        ...
        assertThat(demand.requiredModelCalls()).isEqualTo(390);
    }

    @Test
    void countsVisualInterpretationFromTheImmutablePlanInsteadOfMutableCatalogAvailability() {
        assertThat(policy.demand(plan(19, true)).requiredModelCalls()).isEqualTo(247);
    }
    ...
    void countsEveryPageBindingInsteadOfAssumingOneVisualCallPerSection() {
        ...
        assertThat(policy.demand(plan))
                .isEqualTo(new com.rulepilot.assistant.AssistantRuns.WorkloadDemand(95, 251));
    }

    @Test
    void reservesOneBoundedVisualPassWithoutDependingOnMutableVisualResults() {
        TeachingPlan plan = plan(2, false);

        assertThat(VisualLessonEnricher.maximumTeachingRunModelCalls(plan))
                .isEqualTo(2 * VisualLessonStepLocator.MAX_CANDIDATE_BATCHES_PER_SECTION
                        * VisualLessonStepLocator.MAX_MODEL_CALLS_PER_CANDIDATE_BATCH);
        assertThat(policy.demand(plan).requiredModelCalls()).isEqualTo(22);
    }
