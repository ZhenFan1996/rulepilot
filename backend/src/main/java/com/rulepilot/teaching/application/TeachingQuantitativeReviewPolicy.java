...
/** Identifies rules whose quantities can materially change a legal move, resource ledger, or final score. */
final class TeachingQuantitativeReviewPolicy {
    ...
    static boolean requiresCompleteReviewEvidence(
            ...
            SectionDraft draft) {
        if (planned.coverageTags().stream()
    ...
    private static Stream<String> playerFacingText(SectionDraft draft) {
        return Stream.concat(
                ...
                draft.steps().stream().flatMap(step -> Stream.of(step.heading(), step.text())));
    }
