...
final class AnswerRuleOptionResolver {
    ...
    private RuleOption resolveOne(ModelRequest modelRequest, ModelDraft draft, RuleOptionRequest item) {
        ...
        return new RuleOption(
                AnswerStructuredAidPolicy.requiredText(item.decisionContext(), "option decision context"),
                AnswerStructuredAidPolicy.requiredText(item.selectionRule(), "option selection rule"),
