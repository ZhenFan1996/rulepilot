...
describe('lesson progress continuity', () => {
  it('keeps a known lesson across a transient not-found response', () => {
    const lesson = {
      ...
      sections: [{ evidenceStatus: 'CITED_DRAFT' as const }],
    }
  ...
  it('does not regress the status or discard already published sections', () => {
    const previous = {
      id: 'lesson-1', teachingPlanId: 'plan-1', status: 'COMPLETE' as const,
      sections: [{ evidenceStatus: 'SUPPORTED' as const }, { evidenceStatus: 'SUPPORTED' as const }],
    }
    const stale = {
      id: 'lesson-1', teachingPlanId: 'plan-1', status: 'INCOMPLETE' as const,
      sections: [{ evidenceStatus: 'SUPPORTED' as const }],
    }
    expect(mergeLessonProgress(previous, stale)).toEqual(previous)
  })

  it('does not call an empty or unsupported artifact readable', () => {
    expect(hasReadableLesson({ id: 'empty', teachingPlanId: 'plan-1', status: 'COMPLETE', sections: [] })).toBe(false)
    expect(hasReadableLesson({
      id: 'unsupported', teachingPlanId: 'plan-1', status: 'INCOMPLETE', sections: [{ evidenceStatus: 'INSUFFICIENT_EVIDENCE' }],
    })).toBe(false)
    expect(hasReadableLesson({
      id: 'draft', teachingPlanId: 'plan-1', status: 'DRAFT_READY', sections: [{ evidenceStatus: 'CITED_DRAFT' }],
    })).toBe(true)
  })
})
