...
describe('AppShell', () => {
  ...
  it('shows active work across the app and announces when it ends', async () => {
    ...
    const mobileNavigation = wrapper.get('nav.mobile-navigation')
    expect(mobileNavigation.classes()).toContain('grid-cols-5')
    expect(mobileNavigation.findAll('a')).toHaveLength(5)
    expect(mobileNavigation.find('a[href="/library"]').text()).toContain('公开讲解')
