...
<template>
  <form class="tabletop-panel player-board mt-8 p-5 text-left sm:p-7" @submit.prevent="emit('submit')">
    ...
    <p class="text-sm font-semibold text-muted">{{ t('documents.capture.label') }}</p>
    <div class="mt-3 grid gap-3 sm:grid-cols-3">
      <label for="rulebook-file" class="group flex min-h-32 flex-col rounded-xl border border-dashed border-ink/25 bg-canvas p-4 transition" :class="intakeControlsDisabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-copper/60 hover:bg-copper/5'">
        ...
        <span class="mt-1 text-sm leading-5 text-muted">{{ file?.name ?? t('documents.capture.pdf.detail') }}</span>
      </label>
      <label for="rulebook-camera" class="flex min-h-32 flex-col rounded-xl border border-ink/12 bg-paper p-4 text-ink transition" :class="intakeControlsDisabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-copper/60 hover:bg-copper/[0.1]'">
        ...
        <span class="mt-1 text-sm leading-5 text-muted">{{ t('documents.capture.camera.detail') }}</span>
      </label>
      <label for="rulebook-gallery" class="flex min-h-32 flex-col rounded-xl border border-ink/12 bg-paper p-4 text-ink transition" :class="intakeControlsDisabled ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-indigo/50 hover:bg-indigo/[0.1]'">
        ...
        <span class="mt-1 text-sm leading-5 text-muted">{{ t('documents.capture.gallery.detail') }}</span>
      </label>
    </div>
    ...
    <div v-if="photographedPages.length" class="mt-4 rounded-xl border border-ink/10 bg-canvas p-3 sm:p-4">
      <div class="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <p class="text-xs leading-5 text-muted">{{ t('documents.capture.photoHint') }}</p>
      </div>
      <ol class="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4">
        <li v-for="(page, index) in photographedPages" :key="page.id" class="overflow-hidden rounded-lg border border-ink/10 bg-paper">
          ...
          <div class="flex items-center justify-between gap-1 px-2 py-2">
            <span class="text-xs font-semibold text-muted">{{ t('documents.capture.photoPage', { position: index + 1 }) }}</span>
            <span class="flex gap-1">
              <button type="button" :disabled="intakeControlsDisabled || index === 0" class="rounded px-1.5 py-0.5 text-sm text-muted hover:bg-canvas disabled:opacity-25" :aria-label="t('documents.capture.moveEarlier', { position: index + 1 })" @click="emit('move-photo', index, -1)">←</button>
              <button type="button" :disabled="intakeControlsDisabled || index === photographedPages.length - 1" class="rounded px-1.5 py-0.5 text-sm text-muted hover:bg-canvas disabled:opacity-25" :aria-label="t('documents.capture.moveLater', { position: index + 1 })" @click="emit('move-photo', index, 1)">→</button>
    ...
    <p v-else-if="file" class="mt-3 text-sm text-muted">{{ t('documents.file.change') }} · {{ t('documents.file.limit') }}</p>
    ...
    <label class="mt-4 block text-sm font-semibold">{{ t('documents.title.label') }} <span class="font-normal text-muted">{{ t('documents.optional') }}</span>
      ...
      <span v-if="photographedPages.length" class="mt-1 block text-xs font-normal leading-5 text-muted">{{ t('documents.title.photoHint') }}</span>
    </label>

    <details ref="officialDetails" class="mt-4 border-t border-ink/10 pt-4">
      <summary class="cursor-pointer text-sm font-semibold text-muted">{{ t('documents.advanced') }}</summary>
      <div class="mt-4 stack-y-lg">
        <label class="block text-sm font-semibold">{{ t('documents.source.label') }}
          ...
          <span class="mt-1 block text-xs font-normal leading-5 text-muted">{{ t('documents.source.hint') }}</span>
        </label>
        <div class="rounded-lg border border-indigo/15 bg-indigo/[0.035] p-4">
          ...
          <p class="mt-1 text-xs leading-5 text-muted">{{ t('documents.officialImport.detail') }}</p>
        ...
        <p v-else class="text-sm leading-6 text-muted">{{ t('documents.game.missing') }} <RouterLink :to="{ name: 'catalog' }" class="font-semibold text-indigo underline">{{ t('documents.game.organize') }}</RouterLink>{{ t('documents.game.missingTail') }}</p>
        ...
        <label class="block text-sm font-semibold">{{ t('documents.learningGoal.label') }} <span class="font-normal text-muted">{{ t('documents.optional') }}</span>
          <textarea v-model="learningGoal" data-testid="rulebook-learning-goal" :disabled="intakeControlsDisabled" rows="3" :placeholder="t('documents.learningGoal.placeholder')" class="mt-2 w-full resize-y rounded-lg border border-ink/15 bg-canvas px-4 py-3 font-normal leading-6 outline-none focus:border-copper disabled:opacity-50" />
          <span class="mt-1 block text-xs font-normal leading-5 text-muted">{{ t('documents.learningGoal.hint') }}</span>
        </label>
