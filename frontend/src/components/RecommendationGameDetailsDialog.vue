  ...
  <div v-if="open" class="fixed inset-0 z-50 overflow-y-auto bg-ink/40 px-3 py-6 backdrop-blur-[2px] sm:px-6" @click.self="emit('close')">
    <section ref="dialog" tabindex="-1" class="mx-auto w-full max-w-6xl overflow-y-auto rounded-3xl border border-gold/25 bg-canvas shadow-2xl outline-none sm:max-h-[calc(100vh-3rem)]" role="dialog" aria-modal="true" :aria-label="copy.dialog">
      <header class="app-sticky-top sticky z-10 flex items-start justify-between border-b border-ink/10 bg-paper/95 px-5 py-4 backdrop-blur sm:px-7">
        ...
        <button type="button" data-modal-initial-focus class="grid min-h-11 min-w-11 place-items-center rounded-lg text-2xl text-muted hover:bg-ink/5" :aria-label="copy.close" @click="emit('close')">×</button>
      </header>
      ...
      <div class="grid items-start gap-6 p-4 sm:p-6 lg:grid-cols-[minmax(15rem,21rem)_minmax(0,1fr)] lg:gap-8">
        ...
        <div class="min-w-0">
          <div v-if="loading" data-testid="recommendation-details-loading" class="grid min-h-80 place-items-center text-sm text-muted" role="status">{{ copy.loading }}</div>
          ...
          <template v-else-if="details">
            ...
            <p class="mt-1 text-sm text-muted">{{ details.publicationYear ?? copy.unknownYear }}</p>
            ...
            <dl class="mt-6 grid gap-4 border-t border-ink/10 pt-5 text-sm sm:grid-cols-2">
              <div v-if="details.designers.length"><dt class="font-semibold text-muted">{{ copy.designers }}</dt><dd class="mt-1 leading-6">{{ details.designers.join('、') }}</dd></div>
              <div v-if="details.publishers.length"><dt class="font-semibold text-muted">{{ copy.publishers }}</dt><dd class="mt-1 leading-6">{{ details.publishers.join('、') }}</dd></div>
              <div v-if="details.mechanics.length"><dt class="font-semibold text-muted">{{ copy.mechanics }}</dt><dd class="mt-1 leading-6">{{ details.mechanics.join('、') }}</dd></div>
              <div v-if="details.categories.length"><dt class="font-semibold text-muted">{{ copy.categories }}</dt><dd class="mt-1 leading-6">{{ details.categories.join('、') }}</dd></div>
              <div v-if="details.families.length"><dt class="font-semibold text-muted">{{ copy.families }}</dt><dd class="mt-1 leading-6">{{ details.families.join('、') }}</dd></div>
              <div v-if="details.bestWith || details.recommendedWith"><dt class="font-semibold text-muted">{{ copy.communityFit }}</dt><dd class="mt-1 leading-6">{{ [details.bestWith, details.recommendedWith].filter(Boolean).join(' · ') }}</dd></div>
            </dl>
            <div v-if="details.editionImages.length" class="mt-6 border-t border-ink/10 pt-5">
              ...
              <ul class="mt-3 flex gap-3 overflow-x-auto pb-2"><li v-for="edition in details.editionImages" :key="edition.versionId" class="w-28 shrink-0"><img :src="edition.imageUrl" :alt="edition.name" loading="lazy" class="h-32 w-full rounded-lg bg-paper object-contain"><p class="mt-1 line-clamp-2 text-xs text-muted">{{ edition.name }}</p></li></ul>
            </div>
            <p class="mt-6 rounded-xl bg-indigo/5 px-4 py-3 text-xs leading-5 text-muted">{{ copy.evidence }}</p>
          </template>
        </div>
      </div>
    </section>
