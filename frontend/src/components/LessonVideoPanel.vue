...
<template>
  <section v-if="chapter && activeFrame" class="mt-7" :aria-label="t('video.aria')">
    <div class="relative aspect-[4/3] overflow-hidden rounded-3xl bg-ink-panel text-panel-text elevation-xl sm:aspect-video">
      <div class="relative flex h-full flex-col justify-between p-5 sm:p-8">
        <div class="flex items-start justify-between gap-4">
          ...
          <span class="rounded-full border border-panel-text/20 px-3 py-1 text-xs">{{ t('video.chapter', { position: chapter.position }) }}</span>
        </div>
        ...
        <div>
          ...
          <p v-if="activeFrame.sourcePages.length" class="mt-2 text-center text-xs text-panel-text/60">{{ t('narration.pages', { pages: activeFrame.sourcePages.join(', ') }) }}</p>
        </div>
      </div>
    </div>
    <p class="mt-3 text-sm text-ink/55">{{ chapter.visualCaption }}</p>
    <input class="mt-4 w-full accent-copper" type="range" min="0" :max="durationMillis" :value="playbackMillis" :aria-label="t('video.position')" @input="seek">
    ...
    <div class="mt-3 grid grid-cols-3 gap-2">
      <button :disabled="!audioAvailable" class="min-h-11 rounded-xl bg-copper px-3 text-sm font-semibold text-white disabled:opacity-35" @click="emit('togglePlayback')">{{ audioAvailable ? (playing ? t('video.pause') : t('video.play')) : t('video.audioUnavailable') }}</button>
      <button :disabled="!audioAvailable" class="min-h-11 rounded-xl border border-ink/15 px-3 text-sm font-semibold disabled:opacity-35" @click="emit('replay')">{{ t('video.replay') }}</button>
    ...
    <nav class="mt-4 flex gap-2 overflow-x-auto pb-2" :aria-label="t('video.chapterNav')">
      <button
