          ...
          <div v-show="conversationRole === 'recommendation'">
            <div ref="conversationScroller" data-testid="recommendation-conversation" class="max-h-[76vh] min-h-80 scroll-pb-8 stack-y-md overflow-y-auto px-4 py-5 pb-8 sm:min-h-[38rem] sm:px-6 sm:py-7 sm:pb-9 lg:max-h-[54rem]" aria-live="polite">
              ...
              <article v-for="game in journeyGames" :key="`journey-${game.bggId}`" data-testid="player-journey-continuation" :data-bgg-id="game.bggId" class="overflow-hidden rounded-2xl border border-copper/25 bg-canvas elevation-sm hover:border-copper/45">
                ...
                <div class="flex border-t border-copper/20">
                  <button data-testid="player-journey-progress-button" type="button" class="inline-flex min-h-11 flex-1 items-center justify-center px-3 text-sm font-semibold text-indigo underline" @click="openJourneyProgressCard(game, $event.currentTarget)">{{ t('journeyProgress') }}</button>
                  <button v-if="statusForJourney(game)?.projection.canReadRulebook && statusForJourney(game)?.importJob?.documentVersionId" type="button" class="inline-flex min-h-11 flex-1 items-center justify-center border-l border-copper/20 px-3 text-sm font-semibold text-indigo underline" @click="openJourneyRulebookCard(game, statusForJourney(game)!)">{{ t('journeyReadRulebook') }}</button>
                  <button v-if="statusForJourney(game)?.projection.canReadLesson && statusForJourney(game)?.plan?.id" type="button" class="inline-flex min-h-11 flex-1 items-center justify-center border-l border-copper/20 px-3 text-sm font-semibold text-indigo underline" @click="openJourneyLessonCard(game, statusForJourney(game)!)">{{ t('journeyRead') }}</button>
                  <RouterLink v-if="statusForJourney(game)?.plan?.id" data-testid="player-journey-all-work-link" :to="{ path: '/work', query: { started: statusForJourney(game)?.plan?.id } }" class="inline-flex min-h-11 flex-1 items-center justify-center border-l border-copper/20 px-3 text-sm font-semibold text-ink/55 underline">{{ t('journeyAllWork') }}</RouterLink>
                </div>
              </article>
