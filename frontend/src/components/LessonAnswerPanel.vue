    ...
    <div class="rounded-3xl border border-indigo/20 bg-indigo/[0.035] p-4 sm:p-6">
      <div class="mt-2 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p class="mt-2 text-sm leading-6 text-ink/55">{{ t('lesson.answer.description', { section: currentSection.title }) }}</p>
        </div>
        <span class="rounded-full bg-indigo/8 px-3 py-1.5 text-xs font-semibold text-indigo">{{ t('lesson.answer.sectionContext', { position: currentSection.position }) }}</span>
      </div>
