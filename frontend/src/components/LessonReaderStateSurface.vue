...
<template>
  <section v-if="errorMessage" class="mx-auto max-w-xl px-5 py-20 text-center">
    ...
    <p class="mt-3 text-muted" role="alert">{{ online ? errorMessage : t('lesson.reader.state.error.offline') }}</p>
  ...
  <section v-else class="mx-auto max-w-xl px-5 py-20 text-center">
    ...
    <p class="mt-4 leading-7 text-muted">{{ t('lesson.reader.state.empty.detail') }}</p>
