...
<template>
  <section v-if="errorMessage" class="mx-auto max-w-xl px-5 py-20 text-center">
    ...
    <button v-if="online" class="mt-6 rounded-xl bg-copper px-5 py-3 font-semibold text-white" @click="emit('retry')">{{ t('lesson.reader.state.error.retry') }}</button>
  </section>
  <section v-else class="mx-auto max-w-xl px-5 py-20 text-center">
    ...
    <RouterLink :to="{ name: 'teach' }" class="mt-7 inline-flex rounded-xl bg-copper px-5 py-3 font-semibold text-white">{{ t('lesson.reader.state.empty.action') }}</RouterLink>
  </section>
</template>
