...
describe('LessonView progressive reading', () => {
  ...
  it('presents the persisted cancellation failure code as cancelled rather than failed', async () => {
  ...
  it('does not call an insufficient-evidence placeholder chapter readable', async () => {
    ...
    expect(wrapper.text()).toContain('还没有可读章节')
    const terminal = wrapper.get('[data-failure-classification="repair-required"]')
  ...
  it('shows internal correction guidance for an invalid model-authored plan', async () => {
    vi.stubGlobal('fetch', vi.fn(async (input: string | URL | Request) => {
      const path = String(input)
      if (path === '/api/v1/teaching-plans/plan-1') {
        return Response.json({
    ...
    const terminal = wrapper.get('[data-failure-classification="internal-correction"]')
    expect(terminal.attributes('data-failure-recovery')).toBeUndefined()
    expect(terminal.text()).toContain('内部结构化结果修正已停止')
    expect(terminal.text()).toContain('这不是玩家输入被拒绝')
  ...
  it('uses the same failed terminal presentation after an active run preserves a cited draft', async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (path.includes('mode=TEACHING')) {
        runReads += 1
        const failed = runReads > 1
        const snapshot = runFixture('plan-1', failed ? 'FAILED' : 'VERIFYING_EVIDENCE')
        return Response.json(failed
          ? { ...snapshot, run: { ...snapshot.run, lastErrorCode: 'AGENT_TIMEOUT' } }
          : snapshot)
      }
      if (path.endsWith('/illustrated-lessons/latest')) {
        return Response.json({
    ...
    expect(wrapper.text()).toContain('已保留 1 章可读讲解草稿')
    const terminal = wrapper.get('[data-failure-classification="retry-preserved"]')
    expect(terminal.attributes('data-failure-recovery')).toBe('restart-from-completed')
    expect(terminal.text()).toContain('本轮到达后端记录的截止时间')
  ...
  it('aborts an active generation poll while offline and resumes from a fresh round after reconnect', async () => {
    ...
    expect(teachingReads).toBeGreaterThan(2)
    expect(lessonReads).toBeGreaterThan(2)
    expect(wrapper.text()).toContain('恢复后的完整讲解')
    expect(wrapper.text()).not.toContain('迟到讲解')

    const settledTeachingReads = teachingReads
    const settledLessonReads = lessonReads
    await vi.advanceTimersByTimeAsync(5_000)
    await flushPromises()
    expect(teachingReads).toBe(settledTeachingReads)
    expect(lessonReads).toBe(settledLessonReads)
    wrapper.unmount()
  })
  ...
  it('publishes synchronized chapter visuals from the main teaching snapshots without a visual-enrichment run', async () => {
    ...
    const fetchMock = vi.fn(async (input: string | URL | Request) => {
      const path = String(input)
      if (path === '/api/v1/teaching-plans/plan-1') return Response.json(planFixture('plan-1', '同步配图'))
      if (path.includes('mode=TEACHING')) {
        runReads += 1
        return Response.json(runFixture('plan-1', runReads === 1 ? 'RETRIEVING' : 'COMPLETED'))
      }
      if (path.endsWith('/illustrated-lessons/latest')) {
        lessonReads += 1
        const published = section(1, '第一章')
        return Response.json({
          id: 'lesson-plan-1',
          teachingPlanId: 'plan-1',
          status: lessonReads === 1 ? 'INCOMPLETE' : 'COMPLETE',
          sections: lessonReads === 1
            ? [{ ...published, steps: published.steps.map(step => ({ ...step, visualFocus: null })) }]
            : [published],
        })
      }
      if (path.includes('/pages/') && path.includes('/image/crop?')) return visualImageResponse()
