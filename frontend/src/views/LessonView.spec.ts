...
describe('LessonView progressive reading', () => {
  ...
  it('keeps the reader in place, opens the next published chapter, and unlocks final actions at terminal state', async () => {
    ...
    expect(wrapper.text()).toContain('问这一节的规则')
    expect(wrapper.text()).toContain('这一步哪里不清楚？')
    ...
    const visualStep = wrapper.get('[data-testid="lesson-visual-step"]')
    expect(visualStep.classes()).toContain('min-w-0')
    expect(visualStep.classes()).not.toContain('lg:grid-cols-[minmax(0,1fr)_minmax(19rem,26rem)]')
    expect(visualStep.get('figure').classes()).toContain('max-w-3xl')
