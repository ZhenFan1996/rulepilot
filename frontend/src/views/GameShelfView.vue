...
async function loadShelf() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : t('shelf.error.retry')
  } finally {
    loading.value = false
  }
}
    ...
    <div class="shelf-page mx-auto max-w-7xl px-4 py-7 sm:px-8 sm:py-10 lg:px-12 lg:py-14">
      ...
      <aside class="mt-10 flex flex-col gap-4 rounded-[1.5rem] border border-ink/10 bg-paper p-5 sm:flex-row sm:items-center sm:justify-between">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h2 class="font-semibold">{{ t('shelf.manage.title') }}</h2>
            <p class="mt-1 text-sm text-ink/55">{{ t('shelf.manage.description') }}</p>
          </div>
        </div>
