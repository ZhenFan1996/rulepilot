...
describe('GameWorkspaceView', () => {
  ...
  it('keeps rich game details, rulebooks, teaching, and grounded Q&A in one workspace', async () => {
    ...
    expect(wrapper.get('[data-player-work-stage="RULEBOOK_READY"]').text()).toBe('规则书可读')
  ...
  it('opens a workspace directly from the persisted import before the PDF document exists', async () => {
    ...
    const status = wrapper.get('[data-player-work-stage="ACQUIRING_RULEBOOK"]')
    ...
    expect(wrapper.text()).not.toMatch(/建立检索|building retrieval/i)
  ...
  it('shows a failed exact preparation run instead of claiming it still runs in the background', async () => {
    ...
    expect(wrapper.text()).toContain('讲解准备没有完成')
    expect(wrapper.get('[data-player-work-stage="NEEDS_ACTION"]').text()).toBe('需要处理')
