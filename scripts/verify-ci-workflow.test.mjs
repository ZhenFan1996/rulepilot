...
test('production deployment enables and exercises the bounded recommendation Agent', () => {
  ...
  assert.match(deploymentWorkflow, /totalMillis > 25_000/)
  assert.match(deploymentWorkflow, /openingStreamed\.totalMillis/)
  assert.match(deploymentWorkflow, /streamed\.totalMillis/)
  assert.match(deploymentWorkflow, /BGG_RECOMMENDATION_AGENT_TIMEOUT=PT20S/)
  assert.match(deploymentWorkflow, /AbortSignal\.timeout\(35_000\)/)
  assert.match(deploymentWorkflow, /FALLBACK_VERIFIED_CARDS:/)
