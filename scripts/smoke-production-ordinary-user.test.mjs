...
test('production workflows never execute an operator-supplied Git ref with production credentials', async () => {
  ...
  const availability = await readFile(resolve('scripts/verify-production-availability.mjs'), 'utf8')
  ...
  assert.match(deployment, /name: Verify public release availability/)
  assert.match(deployment, /node scripts\/verify-production-availability\.mjs/)
  assert.match(availability, /`\$\{origin\}\/api\/auth\/csrf`/)
  assert.match(availability, /\/api\/v1\/bgg\/recommendations/)
  assert.match(availability, /\/api\/v1\/bgg\/games\/\$\{firstGame\.bggId\}/)
  assert.match(productionCompose, /OUTBOX_PUBLISH_FIXED_DELAY: "500ms"/)
  assert.match(productionCompose, /TEACHING_IMPORT_HANDOFF_FIXED_DELAY: "500ms"/)
  assert.match(productionCompose, /TEACHING_STARTUP_QUEUE_CAPACITY: "3"/)
  assert.match(productionCompose, /TEACHING_BACKGROUND_CORE_POOL_SIZE: "1"/)
  assert.match(productionCompose, /TEACHING_BACKGROUND_MAX_POOL_SIZE: "1"/)
