...
用户可以先导入规则书，也可以先建立游戏资料。RulePilot 读取规则正文、示例、图片和表格；规则书关联到游戏版本后，即可生成一套可逐步学习、可追溯到原始页码的完整讲解。
...
登录后可以从任一入口开始：打开 http://127.0.0.1:5173/teach 直接上传并解析规则书，不必先创建游戏；或者打开 http://127.0.0.1:5173/catalog 创建、从 BGG 导入游戏和版本。尚未归属的规则书会保留在个人规则书列表，之后关联版本即可开始讲解。

### 从 BoardGameGeek 读取游戏信息

RulePilot 可以在游戏目录中直接搜索 BoardGameGeek（BGG），并导入游戏名称、发行年份、人数、时长、建议年龄、简介、缩略图和原始页面链接。导入记录保存在本地 PostgreSQL；相同 BGG ID 不会重复创建游戏。BGG 对 API 使用要求先注册并获批应用，因此请前往 https://boardgamegeek.com/applications 获取 Application Token，然后写入不会提交到 Git 的 `.env`：
...
配置 Token 后，首页还会显示 BGG 当前热门桌游的封面、排名和来源链接；热门列表缓存 1 小时。没有 Token 时，首页优先展示当前账号已导入的 BGG 游戏封面，并提供连接提示。

BGG 元数据只用于目录识别，不进入规则讲解、Agent 或 RAG 证据。使用本功能前请确认场景符合 BGG API 的非商业条款；界面保留 BGG 来源链接和 Powered by BGG 标识。

打开 http://127.0.0.1:5173/teach，可以上传 50 MiB 以内的 PDF 规则资料。“关联游戏”是可选项：暂不选择时只上传、解析并保存到个人规则书列表；从游戏页进入时会预选版本并在解析后直接开始讲解。文件会流式保存到 MinIO，在 PostgreSQL 中记录版本与 SHA-256，并在后台通过 PDFBox 按原始页码提取文字；页面通过 SSE 显示实时进度。只有属于当前用户、解析完成且已关联游戏版本的规则书可以创建教学计划。之后用户可按玩家人数、新手人数和时长进入图文讲解页逐章学习；缺失或尚不可评估的规则证据会持续显示，不会补写无依据内容。
