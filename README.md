...
图文讲解由 Teaching Agent 逐章执行版本隔离的混合检索和结构化生成。默认 `TEACHING_PROVIDER=fake` 便于本地无外部费用运行；接入可用的 Spring AI `ChatModel` 后可切换为 `spring-ai`。无论使用哪个模型提供方，引用白名单、长度、章节结构和工具调用预算都由应用层校验。
