...
用户可以先导入规则书，也可以先建立游戏资料。RulePilot 读取规则正文、示例、图片和表格；没有关联游戏时会根据规则书标题自动创建游戏和默认版本，然后直接生成一套可逐步学习、可追溯到原始页码的完整讲解。
...
打开 http://127.0.0.1:5173/teach，可以上传 50 MiB 以内的 PDF 规则资料。“关联游戏”是可选项：暂不选择时，文件解析完成后会自动创建目录资料并继续生成讲解；从游戏页进入时则会预选已有版本。文件会流式保存到 MinIO，在 PostgreSQL 中记录版本与 SHA-256，并在后台通过 PDFBox 按原始页码提取文字；页面通过 SSE 显示实时进度。教学计划只要求规则书属于当前用户且解析完成，目录归属由后端自动补齐。之后用户可按玩家人数、新手人数和时长进入图文讲解页逐章学习；缺失或尚不可评估的规则证据会持续显示，不会补写无依据内容。
